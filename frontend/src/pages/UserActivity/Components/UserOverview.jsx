...
export default function UserOverview() {
    ...
    // const sortedUserPosts = userActivity?.userPosts?.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    
    // const sortedUserComments = userActivity?.userComments?.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

    const combinedActivity = [
        ...(userActivity?.userPosts?.map(post => ({ ...post, type: 'post' })) || []),
        ...(userActivity?.userComments?.map(comment => ({ ...comment, type: 'comment' })) || [])
                            ...
                            <ListItemText
                              ...
                              primary={
                                <>
                                  ...
                                  <Tooltip
                                    title={new Date(activity.createdAt).toLocaleDateString(
                                      "en-US",
                                      {
                                        month: "long",
                                        day: "numeric",
