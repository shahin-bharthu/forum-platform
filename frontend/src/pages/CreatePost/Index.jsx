...
const CreatePost = ({ isEdit }) => {
  ...
  const { forumName, forumId } = location.state || {
    forumName: "",
    forumId: "",
  };
  ...
  const topicDetailsLoader = async (topicId) => {
    if (topicId) {
      const response = await axiosInstance.get(
        `http://localhost:8080/topic/${topicId}`,
  ...
  return loading && isEdit ? (
    ...
    >
      ...
      <form
        onSubmit={
          isEdit
            ? (event) => editPostHandler(event, topicData.topic.id)
            : addPostHandler
        }
        noValidate
      >
        {!isEdit && (
          <SelectList
            ...
            getForum={getSelectedForumFromList}
          />
        )}
