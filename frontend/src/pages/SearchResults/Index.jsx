...
const SearchResults = () => {
  ...
  const { forumResults, postResults } = location.state || {
    forumResults: [],
    postResults: [],
  };
  const [searchForumsResults, setSearchForumsResults] = useState([
    {
      ...
      purpose: "Try searching for something else",
      avatarUrl: "",
    },
  ]);
  ...
  const [searchCommentsResults, setSearchCommentsResults] = useState([
    {
      comment: {
        content: "Double-check your spelling or try different keywords",
      },
      topic: { id: "" },
      forum: {},
      user: { username: "No results found" },
      avatar: "",
    },
  ]);
      ...
      const commentsData = await Promise.all(
        results.map(async (comment) => {
          const commentResponse = (
            await axiosInstance.get(`comment/id/${comment.id}`)
          ).data;
          const userAvatar = (
            await axiosInstance.get(
              `/user/avatar/${commentResponse.data.user.id}`,
              { responseType: "blob" }
            )
          ).data;
          return {
  ...
  const getForumAvatars = async (forumsList) => {
    const forumData = await Promise.all(
      forumsList.map(async (forum) => {
        if (forum.id) {
          const forumAvatarPath = await axiosInstance.get(
            `/forum/banner/${forum.id}`,
            { responseType: "blob" }
          );
    ...
    const topicsData = await Promise.all(
      topicsList.map(async (topic) => {
        if (topic.id) {
          ...
          const topicCreatorAvatar = (
            await axiosInstance.get(`/user/avatar/${topicRes.data.user.id}`, {
              responseType: "blob",
            })
          ).data;
          return {
  ...
  return (
    <Box sx={{ width: "100%", mt: 10, alignSelf: "start" }}>
      ...
      <CustomTabPanel value={value} index={2}>
        <List sx={{ width: "100%" }}>
          {searchCommentsResults.length > 0 ? (
            searchCommentsResults.map((result) => {
              return (
                <>
                  <Link
                    key={result.id}
                    to={`/post/${result.topic.id}`}
                    state={{parentId: result.comment.parent_comment_id, commentId: result.comment.id}}
